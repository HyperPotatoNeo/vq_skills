...
def franka_plan_cost_fn(prev_states,skill_idx,model,prior_search=False,ep_tasks=None):
    '''
    ...
    idx_list = skill_idx.copy()
    skill_seq = torch.zeros((batch_size,H,model.vector_quantizer.z_dim)).cuda()
    s = torch.stack(batch_size*[torch.tensor(prev_states[-1:,:],device=torch.device('cuda:0'),dtype=torch.float32)])
    for t in range(H):
        # print('=================================================')
        # get predicted next state
        if(prior_search):
            _,idx_dist = model.prior(s)
            cat_dist = Categorical.Categorical(torch.squeeze(idx_dist,dim=1))
            idx_sample = cat_dist.sample().cpu().numpy()
            idx_list[:,t] = idx_sample

            skill_seq[:,t,:] = model.vector_quantizer.embedding.weight[idx_sample]
        else:
            skill_seq[:,t,:] = model.vector_quantizer.embedding.weight[skill_idx[:,t]]
        z_t = skill_seq[:,t:t+1,:]
    ...
    if prior_search:
        return idx_list,-torch.tensor(cum_rew,device=torch.device('cuda:0'),dtype=torch.float32)
